...
GLYPH_FONT_FAMILY="Consolas" # Set to your desired symbols font
...
#Function to get artist, cuz Im too lazy
def get_current_artist():
    try:
        result = subprocess.run([PLAYERCTL_PATH, 'metadata','title','artist'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        artist = result.stdout.decode('utf-8').strip()
        if result.returncode != 0 or not artist:
            return None  # Return None if no artist
        return artist
    except Exception as e:
        return None
